...
zotlook-prefs-contactsheet-menustay-help = A menu stays open after an entry was followed, so that the next entry is a click or an Enter away. Unticked, it folds on the jump so that the page can be seen.
zotlook-prefs-contactsheet-menustay-mouse =
    .label = Keep the menu open after a click on an entry
zotlook-prefs-contactsheet-menustay-keyboard =
    .label = Keep the menu open after Enter on an entry
