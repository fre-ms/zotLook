...
- The window says that the sheet is on its way while it is still empty, and
  ...
  opened; fixed.
- A menu stays open after an entry was followed, by mouse and by keyboard
  alike, so that the next entry is a click or an Enter away; two settings
  fold it on the jump instead.
...
- On GNOME under X11 Sushi's preview window now stands above the Zotero
  ...
  focus stealing. Under Wayland it always stood in front.
- A page field in the window's top left corner, Ctrl+G, takes a page number
  ...
  pages alone, in as many columns as fit them into the window.
...
- Roman numerals typed land in the page field as digits do, on a sheet whose
  ...
  search.
- "o" opens the framed page in the reader with the contents menu open too;
  before, it only jumped to the tile a second time there, since a contents
  entry carries no reader link of its own.
