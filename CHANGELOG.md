...
- **Keyboard.** Enter frames the next page and Shift+Enter the previous, the
  arrows move the frame across the grid, o or Ctrl+Enter opens the framed
  page in the reader, and c and a open the contents and the annotations,
  where the arrows walk the entries and Enter follows one. Page Up and Page
  Down scroll, as before.
