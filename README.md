...
The code is split into three scripts, loaded in that order into the plugin scope: `util.js` (parsing, sanitising and URL helpers), `epub.js` (the EPUB-to-HTML pipeline) and `zotlook.js` (windows, shortcuts, subprocesses, notes and contact sheets). Strings live in `locale/<locale>/zotlook.ftl`.
